...
class StructureRuleEngine:
    ...
    def classify_lines(
        ...
        if has_position:
            header_group, body_group, footer_group = split_pdf_regions(lines, page_height)
        else:
    ...
    def _classify_header(self, lines: list[dict[str, Any]]) -> list[ClassifiedLine]:
    ...
    def _try_header_field(self, line: dict[str, Any], text: str) -> ClassifiedLine | None:
        ...
        if self._is_large_font(
            ...
            is_heading_font = any(
                kw in font_family for kw in ("黑体", "方正小标宋", "宋体", "华文中宋")
            )
            ...
            if logo_confidence >= 0.6:
                return self._build_classified(line, "issuing_logo", logo_confidence)

        return None

    def _classify_body(self, lines: list[dict[str, Any]]) -> list[ClassifiedLine]:
    ...
    def _classify_outline_level(self, line: dict[str, Any]) -> tuple[str, float]:
        ...
        for rules in (HEADING_RULES, HEADING_RULES_OCR):
            for outline_level, pattern, expected_font, expected_bold in rules:
                if pattern.match(text):
                    ...
                    if "。" in text and len(text) > 30 and confidence < 0.55 and not font_weight:
                        continue
    ...
    def _classify_footer(self, lines: list[dict[str, Any]]) -> list[ClassifiedLine]:
    ...
    def _try_footer_field(self, line: dict[str, Any], text: str) -> ClassifiedLine | None:
        ...
        if DISTRIBUTION_DATE_PATTERN.search(text):
            ...
            return self._build_classified(line, "distribution_date", date_conf)
    ...
    def _is_likely_title_line(self, lines: list[dict[str, Any]], idx: int) -> bool:
