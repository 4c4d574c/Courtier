        ...
        try:
            ...
            if attempt < max_retries:
                ...
                logger.warning(
                    ...
                    description, attempt + 1, total_attempts, delay, exc,
                )
                time.sleep(delay)
            else:
                logger.error(
                    "%s failed after %d attempts: %s",
                    description, total_attempts, exc,
                )
    raise RuntimeError(
...
分类依据（同时参考文本编号格式、字体特征和段落样式）：
- **heading1**：中文数字编号段落，如"一、""二、""三、"开头的标题行，
  通常使用黑体加粗；如果段落样式名为"Heading 1"也属此类
  **重要：一级标题必须加粗。如果该行未加粗（即输入中无[加粗]标记），即使文本匹配此格式，也必须归类为 body_text。**
- **heading2**：带括号中文数字编号段落，如"（一）""（二）""（三）"开头的标题行，
  通常使用楷体加粗；如果段落样式名为"Heading 2"也属此类
  **重要：二级标题必须加粗。如果该行未加粗（即输入中无[加粗]标记），即使文本匹配此格式，也必须归类为 body_text。**
- **heading3**：阿拉伯数字加点编号段落，如"1.""2.""3."开头的标题行，
  通常使用仿宋加粗；如果段落样式名为"Heading 3"也属此类
  **重要：三级标题必须加粗。如果该行未加粗（即输入中无[加粗]标记），即使文本匹配此格式，也必须归类为 body_text。**
- **heading4**：带括号阿拉伯数字编号段落，如"（1）""（2）""（3）"开头的标题行，
  通常使用仿宋；如果段落样式名为"Heading 4"也属此类
- **heading5**：其他层级的标题行
- **body_text**：无编号的正文段落内容，或虽带编号标题格式但未加粗的行
- **others**：无法归类的文本
...
class LLMClient:
    ...
    def recognize_fonts(
        ...
        """Call multimodal LLM to identify font properties from document image.

        Sends the document page image along with OCR-extracted text lines
        to the multimodal LLM, asking it to identify the font family,
        weight, and style for each line based on visual appearance.
        ...
        text_prompt = (
            "以下是从文档中提取的文本行：\n\n"
            f"{numbered_text}\n\n"
            "请结合文档图片，识别每一行文字的字体属性"
            "（字体名称、是否加粗、是否倾斜）。"
        )

        logger.info(
            "Calling font recognition LLM with %d lines and image %s",
            len(lines),
            image_path,
        )
        ...
        response = _call_with_retry(
            ...
            description="Font recognition LLM API call",
        )
        ...
        logger.info(
            "Font recognition completed: %d lines identified", len(font_info)
        )
        return font_info
    ...
    def recognize_structure_text_only(
        self,
        lines: list[dict[str, Any]],
        partial_result: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        """Call LLM with text only (no image) for structure recognition.

        Used as fallback when the rule engine has low confidence on some
        lines. Only sends uncertain lines with surrounding context.

        Args:
            lines: Lines that need LLM classification.
            partial_result: Already-classified fields from rule engine,
                used as context for the LLM.

        Returns:
            Parsed JSON structure for the uncertain lines.
        """
        numbered_text = _format_lines(lines)
        ...
        if partial_result:
            ...
            if classified_fields:
                context_hint = (
                    f"\n\n已知分类字段：{', '.join(classified_fields)}。"
                    "请仅对未分类的行进行分类。"
                )

        text_prompt = (
            "以下是从文档中提取的文本行（部分可能已被分类）：\n\n"
            f"{numbered_text}{context_hint}\n\n"
            "请识别这些文本行所属的公文结构（版头/主体/版记）。"
        )

        logger.info(
            "Calling text-only LLM with %d lines (fallback)", len(lines)
        )
        ...
        response = _call_with_retry(
            ...
            description="Text-only structure recognition LLM API call",
        )
