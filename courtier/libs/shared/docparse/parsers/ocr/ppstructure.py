...
class PPStructureAdapter:
    ...
    def recognize(self, image_path: str) -> OCRPageResult:
        ...
        try:
            ...
            raise RuntimeError(
                f"PPStructureV3 API call failed: {exc}"
            ) from exc
...
def _parse_lines(page_data: dict[str, Any]) -> list[OCRLineResult]:
    ...
    """
    rec_texts, rec_scores, rec_boxes, rec_polys, text_word, text_word_boxes = \
        _get_ocr_arrays(page_data)
...
def _merge_same_line_items(
    ...
    short_widths = sorted(
        line.x1 - line.x0 for line in lines
        if 0 < (line.x1 - line.x0) <= char_height * 2.5
    )
    if short_widths:
...
def _merge_line_group(
    group: list[OCRLineResult], sep: str = "",
) -> OCRLineResult:
    ...
    avg_confidence = (
        sum(line.confidence for line in sorted_group) / len(sorted_group)
    )
...
def _join_blocks_with_tabs(
    ...
    avg_confidence = (
        sum(b.confidence for b in sorted_blocks) / len(sorted_blocks)
    )
