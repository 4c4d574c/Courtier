...
def parallel_ocr(
    ...
    if len(image_paths) == 1:
        try:
            ...
            logger.warning("OCR failed for page 0: %s", exc)
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(ocr_engine.recognize, img): idx
    ...
    return [
        r if r is not None else OCRPageResult(lines=[], width=0, height=0)
        for r in results
    ]
...
def ocr_result_to_lines(page_result: OCRPageResult) -> list[dict[str, Any]]:
    ...
    for line in page_result.lines:
        ...
        alignment = compute_alignment_from_position(
            line.x0, line.x1, page_result.width, outline_level,
        )
        font_size = compute_font_size_from_ocr(
            line.y1 - line.y0, page_result.height,
        )
