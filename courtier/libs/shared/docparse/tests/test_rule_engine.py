...
class TestPdfRegionSplit:
    def test_basic_split_by_y(self) -> None:
        ...
        assert len(header) >= 1 or len(body) >= 3, (
...
class TestRuleEngineFullPage:
    ...
    def test_low_confidence_detection(self) -> None:
        """Lines with no clear signal should have low confidence."""
        lines = [
            _make_line("这是一行不明确的内容", 0),
        ]
        result = self.engine.classify_lines(lines, has_position=False)
        # This line has no header markers, no footer markers → goes to body
        # but has no title/heading/attachment patterns → body_text with low conf
        low = result.low_confidence_lines(threshold=0.8)
        assert len(low) >= 1


class TestClassifyResultHelpers:
    def test_all_confident_true(self) -> None:
        result = ClassifyResult(
            lines=[
                ClassifiedLine(line_no=0, text="t", field="title", confidence=0.9),
                ClassifiedLine(line_no=1, text="b", field="body_text", confidence=0.85),
            ]
        )
        assert result.all_confident(0.8)

    def test_all_confident_false(self) -> None:
        result = ClassifyResult(
            lines=[
                ClassifiedLine(line_no=0, text="t", field="title", confidence=0.9),
                ClassifiedLine(line_no=1, text="b", field="body_text", confidence=0.5),
            ]
        )
        assert not result.all_confident(0.8)

    def test_unclassified_skipped_in_confidence_check(self) -> None:
        result = ClassifyResult(
            lines=[
                ClassifiedLine(line_no=0, text="t", field="title", confidence=0.9),
                ClassifiedLine(
                    line_no=1, text="?", field="unclassified", confidence=0.0
                ),
            ]
        )
        assert result.all_confident(0.8)
...
class TestOcrResilientPatterns:
    ...
    def test_issuing_number_ocr_matches(self, text: str) -> None:
        assert ISSUING_NUMBER_PATTERN_OCR.match(text), (
            f"OCR pattern should match: {text!r}"
        )
...
class TestRuleEngineScannedPDF:
    ...
    def test_full_scanned_page_classification(self) -> None:
        ...
        result = self.engine.classify_lines(
            lines, has_position=True, page_height=841.89
        )
...
class TestHeadingBodyTextDowngrade:
    ...
    def test_heading2_long_body_paragraph_in_classify_body(self) -> None:
        ...
        result = self.engine._classify_body(lines)
        assert result[0].field == "body_text", (
            f"Expected body_text, got {result[0].field!r}"
        )
    ...
    def test_heading3_long_body_paragraph_downgrade(self) -> None:
        ...
        outline, conf = self.engine._classify_outline_level(line)
        assert outline == "", (
            f"Expected empty (downgraded), got {outline!r} with conf={conf}"
        )
    ...
    def test_heading3_short_title_kept(self) -> None:
        ...
        outline, conf = self.engine._classify_outline_level(line)
        assert outline == "heading3", (
            f"Expected heading3, got {outline!r} with conf={conf}"
        )
    ...
    def test_heading4_short_title_kept(self) -> None:
        ...
        outline, conf = self.engine._classify_outline_level(line)
        assert outline == "heading4", (
            f"Expected heading4, got {outline!r} with conf={conf}"
        )
