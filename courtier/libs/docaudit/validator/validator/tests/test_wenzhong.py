...
class TestDetectWenzhongByBody:
    ...
    def test_body_keyword_match(self, title: str, body_text: str, expected: str) -> None:
...
class TestDetectWenzhongFromDoc:
    ...
    def test_doc_with_title(self) -> None:
        ...
        title_para = Paragraph(elements=[LineElement(font=Font(text="国务院关于XX的通知"))])
        body_para = Paragraph(elements=[LineElement(font=Font(text="现通知如下。"))])
        page = Page(page_content=PageContent(body=Body(title=title_para, main_text=[body_para])))
