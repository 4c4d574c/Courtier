...
try {
  ...
  //    text starts a fresh turn, and the SSE request carries editTurn.
  {
    ...
    assert.ok(url.includes("editTurn=1"), url);
    assert.ok(url.includes("sessionId=sess_abc"), url);
    assert.ok(url.includes(encodeURIComponent("改后的第二问")), url);
    assert.ok(!url.includes("fileId="), url);
  }
  ...
  // 2. Editing the first turn keeps its upload and retitles the session.
  {
    ...
    const url = esInstances[0].url;
    assert.ok(url.includes("editTurn=0"), url);
    assert.ok(url.includes("fileId=f1"), url);
  }
