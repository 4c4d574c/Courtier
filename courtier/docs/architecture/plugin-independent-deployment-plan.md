...
| 阶段四：transport 按部署距离抽象 | ✅ 已落地（2026-08-17，超范围完成） | stdio spawn 整体删除，插件改造为独立 TCP 服务（主进程拨号 + 双向 token 鉴权 + 无限退避重连）；文件经 MinIO transfer bucket 双向传递（插件持受限凭据）；8 插件独立 Docker 镜像与 compose 服务 | 见 `docs/superpowers/plans/2026-08-17-plugin-standalone-deployment-implementation.md` 的实施记录与总验收 |
