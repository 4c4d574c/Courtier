...
- 2026-09-03（下午，用户真机反馈驱动）：
  ...
  - 端点变迁：`.22:8006`（Qwen3.8-27B）已不可访问，按用户指示池已切回 `.67:8001 / Qwen3.5-27B`（modalities 维持 vision+video）。引擎恢复后**视频直通最后一环补验通过**（2026-09-03 15:48）。
  - **visual_inspection 扩为图片+视频双模态**（用户指示）：`VisualInspectionInput` 的 image/video 字段均为 `Optional[MediaRef 子类]`（model_validator 强制二选一）；`SkillTool._media_fields` 支持解包 Optional 联合（PEP 604）；技能正文补充视频观察指引。真机验收：webm 上传自动转码 → activate_domain → visual_inspection（video=file_id 正确传入）→ 子代理内联观察到关键帧并详尽描述录屏内容（识别出公文审核平台演示的操作流程与审核结果）。附带修正：resolver 的 MIME 改按存储文件名（转码后实际为 mp4，原按显示名误标 webm）。
