...
class RuleFile(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default="uploaded", comment="状态：uploaded, processed, failed")
...
class Rule(Base):
    ...
    pattern: Mapped[str] = mapped_column(Text, nullable=False, comment="规则模式（正则表达式或其他格式）")
    severity: Mapped[str] = mapped_column(String(20), default="warning", comment="严重程度：error, warning, info")
    domain_id: Mapped[str] = mapped_column(String(20), nullable=False, comment="所属领域ID（如GOV、AUD等）")
    rule_file_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("rule_file.id"), comment="来源文件")
