...
class AgentResult:
    ...
    @classmethod
    def from_state(
        cls, state: AgentState, *, include_state: bool = False
    ) -> "AgentResult":
...
class Agent:
    ...
    def __init__(
        ...
        # Inject project-level rules (DRUDGE.md) if provided
        if drudge_md_content:
            self._prompt_pipeline.set_drudge_md(drudge_md_content)
    ...
    async def run(
        ...
        if context_manager is not None:
            self._prompt_pipeline.set_context_instructions(
