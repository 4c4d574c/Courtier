...
BEHAVIORAL_RULES = (
    ...
    "  · 好：「文档已解析，现进行格式审核。」\n"
    "- 【关键】$ref 引用使用规则：\n"
    "  · $ref 引用（工具输出中的 result_id 字段）只能从工具返回结果的 metadata 中获取，"
    "禁止自行编造或推断；\n"
    ...
    "- 【关键】search_documents 引用标记规则：\n"
    "  · 引用标记 [[n]] 必须紧跟在实际被引用的具体内容之后（某条款、某句或某短语），"
    "标注在引用位置；禁止把标记统一堆到段落或列表项末尾，禁止在一处集中堆叠多个标记。"
    "引用或摘录结果中的条文、条款、原文时同样必须标注。"
    "n 为该次搜索结果 hits 列表的序号（从 1 开始，第 1 条命中为 [[1]]）；\n"
    "  · 示例：\n"
    "    差：库中有《数据安全法》，涵盖数据安全与发展、数据安全制度等章节[[1]]。\n"
    "    好：库中有《数据安全法》[[1]]，涵盖数据安全与发展、数据安全制度等章节。\n"
    "    差：涉及数据分类分级保护、数据安全风险评估等配套条款[[2]][[3]]。\n"
    "    好：涉及数据分类分级保护[[2]]、数据安全风险评估[[3]]等配套条款。\n"
    "  · 只引用结果中实际存在的序号，禁止编造或推断不存在的命中；\n"
...
PERIODIC_REMINDER = (
    ...
    "引用 search_documents 结果时用 [[n]] 标注在实际引用位置。"
)
...
PRE_TURN_REMINDER = (
    ...
    "本轮若引用 search_documents 结果（含摘录条文原文），"
    "必须用 [[n]] 标注并紧跟在实际引用内容之后。"
)
