...
```
courtier/                          # 项目根目录（实际路径可能是 docaudit-agent/）
├── packages/                      # 领域无关引擎 + 领域包 + 前端
│   ├── core/                      # Courtier 通用引擎（领域无关）
│   │   └── src/courtier/
...
│   │       ├── plugins/          # JSON-RPC 插件子进程
│   │       │   ├── docparse/     # 文档解析
│   │       │   ├── validator/    # 格式校验
│   │       │   ├── content_compliance/  # 内容合规
│   │       │   ├── doccorrector/ # 文本纠错
│   │       │   ├── docannot/     # 文档标注
│   │       │   └── audit/        # 审计包装插件（format_audit, content_audit, text_correction, plagiarism）
│   │       └── skills/           # Skill 文档（Markdown + YAML frontmatter）
│   └── webui/                    # Vue 3 + Vite 前端（Terminal-style UI）
├── src/                          # 遗留模块（已清理；docmodels 已迁移至 domain 包）
├── tests/                        # 测试套件
│   ├── agent/                    # 核心引擎/API 测试
...
# 运行 API 服务
PYTHONPATH=packages/core/src:packages/domains/docaudit:packages/domains/docaudit/plugins:src \
...
1. **API 层** (`packages/core/src/courtier/agent/api/`): HTTP handlers (`routes/`), SSE 流 (`sse_adapter.py`), 文件/会话存储, 可观测性中间件
2. **服务层** (`packages/core/src/courtier/agent/api/services/`): AgentService, StreamService, FileService, SessionService
3. **核心引擎层** (`packages/core/src/courtier/agent/`): Agent loop, PluginSystem, ToolRegistry, Artifact 系统, Context 管理, Prompt pipeline, Hooks, Permissions, Telemetry
4. **领域/业务层** (`packages/domains/docaudit/plugins/`): docparse, validator, content_compliance, doccorrector, docannot 等旧模块作为共享库被 `common/*` 和 `audit/*` JSON-RPC 包装插件调用；对外暴露的插件为 parse, template, search, annotate, plagiarism, format_audit, content_audit, text_correction
...
- `plugins_path` / `skills_path` — 文件系统路径
...
- `src/` 下遗留模块已基本清理；`docmodels` 已迁移至 `packages/domains/docaudit/docmodels/`
- `packages/core/src/courtier/agent/api/app.py` 中直接导入 `content_compliance.init_checkers()` 的泄漏已移除；初始化由 `content_audit` 插件延迟完成
- 可观测性配置（docker-compose、Prometheus、Grafana）已从 `docaudit` 品牌统一重命名为 `courtier`
- Dockerfile 引用路径已更新为 monorepo 结构，但尚未在 CI 中验证完整构建与前端静态文件服务
