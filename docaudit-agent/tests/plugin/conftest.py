...
_PLUGINS_ROOT = Path(__file__).resolve().parents[2] / "packages" / "domains" / "docaudit" / "plugins"


def _plugin_dir(name: str) -> str:
    """Return the absolute path to a plugin directory.

    Supports flat (plugins/name/) and nested (plugins/common/name/,
    plugins/audit/name/) layouts.
    """
    direct = _PLUGINS_ROOT / name
    if direct.is_dir():
        return str(direct)
    for sub in ("common", "audit"):
