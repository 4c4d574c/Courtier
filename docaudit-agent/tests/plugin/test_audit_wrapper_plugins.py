"""Tests for JSON-RPC wrapper plugins in plugins/audit/.
...
async def test_format_audit_plugin_registers_check_format():
    ...
    from plugins.audit.format_audit.entry import FormatAuditPlugin
...
async def test_format_audit_tool_validates_doc():
    ...
    from plugins.audit.format_audit.tools import FormatAuditTool

    tool = FormatAuditTool()
    sample_result = [{"block": "title", "error": "font_mismatch", "detail": "expected 22pt"}]

    with patch("plugins.audit.format_audit.tools.validator", return_value=sample_result):
...
async def test_format_audit_tool_handles_error():
    ...
    with patch(
        "plugins.audit.format_audit.tools.validator",
...
async def test_content_audit_plugin_registers_check_content():
    ...
    from plugins.audit.content_audit.entry import ContentAuditPlugin
...
async def test_content_audit_tool_returns_violations():
    ...
    from plugins.audit.content_audit.tools import ContentAuditTool
    ...
    with patch(
        "plugins.audit.content_audit.tools.get_checker", return_value=mock_checker
    ):
...
async def test_content_audit_tool_unknown_doc_type():
    ...
    with patch("plugins.audit.content_audit.tools.get_checker", return_value=None):
...
async def test_text_correction_plugin_registers_correct_text():
    ...
    from plugins.audit.text_correction.entry import TextCorrectionPlugin
...
async def test_text_correction_tool_returns_results():
    ...
    from plugins.audit.text_correction.tools import TextCorrectionTool

    tool = TextCorrectionTool()
    mock_corrections = [{"source": "测试文本", "target": "测试文本", "errors": []}]

    with patch(
        "plugins.audit.text_correction.tools.ErrorCorrect",
...
async def test_plagiarism_plugin_registers_detect_plagiarism():
    ...
    from plugins.audit.plagiarism.entry import PlagiarismPlugin
