...
def _set_refresh_cookie(
    ...
    # Use the actual request scheme (or X-Forwarded-Proto from a reverse proxy)
    # to decide whether the cookie should be Secure. This avoids dropping the
    # refresh cookie during local HTTP development when DEPLOYMENT_ENVIRONMENT
    # is accidentally left as "production".
    forwarded_proto = request.headers.get("x-forwarded-proto")
    secure = forwarded_proto == "https" or (
...
def _fallback_login(body: LoginRequest, settings) -> str:
    ...
    if body.username != admin_user or not secrets.compare_digest(
