...
from sqlalchemy import Float, Integer, DateTime, ForeignKey, Enum
...
# 大纲等级枚举
OUTLINE_LEVEL_ENUM = ["heading1", "heading2", "heading3", "heading4", "heading5", "body_text", "others"]

class ParagraphCreate(BaseModel):
    """创建段落时的数据模型"""

    page_id: int = Field(..., description="所属页面ID")
    section_type: str = Field(..., description="段落类型（所属区域）", json_schema_extra={"enum": SECTION_TYPE_ENUM})
    ...
    outline_level: str = Field(..., description="大纲等级", json_schema_extra={"enum": OUTLINE_LEVEL_ENUM})
...
class ParagraphTable(Base):
    ...
    page_id: Mapped[int] = mapped_column(Integer, ForeignKey("pages.id", ondelete="CASCADE"), nullable=False, comment="所属页面ID")
    section_type: Mapped[str] = mapped_column(Enum(*SECTION_TYPE_ENUM), nullable=False, comment="段落类型")
    order_index: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="同一类型内的顺序")
    ...
    left_indent: Mapped[float] = mapped_column(Float, nullable=False, default=0.0, comment="文本之前缩进(pt)")
    right_indent: Mapped[float] = mapped_column(Float, nullable=False, default=0.0, comment="文本之后缩进(pt)")
    block_no: Mapped[int] = mapped_column(Integer, nullable=False, comment="块序号")
    outline_level: Mapped[str] = mapped_column(Enum(*OUTLINE_LEVEL_ENUM), nullable=False, comment="大纲等级")
    create_time: Mapped[datetime] = mapped_column(DateTime, default=utcnow, comment="创建时间")

    # 关系
    page: Mapped["PageTable"] = relationship(back_populates="paragraphs")  # noqa: F821
    elements: Mapped[list["ElementTable"]] = relationship(back_populates="paragraph", cascade="all, delete-orphan")  # noqa: F821
